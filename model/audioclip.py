...
class AudioCLIP(CLIP):
    ...
    def __init__(self,
        ...
        if isinstance(self.pretrained, str):
            ...
            weights_path = '../assets/AudioCLIP-Full-Training.pt'  # 경로 수정
            self.audio.load_state_dict(torch.load(weights_path, map_location='cpu'), strict=False)
            print('Image & Text weights loaded')

            try:
                audio_weights_path = '../assets/ESRNXFBSP.pt'  # 경로 수정
                self.audio.load_state_dict(torch.load(audio_weights_path, map_location='cpu'), strict=False)
                print('Audio weights loaded from custom path')
    ...
    #     # print(f'Audio embedding shape: {audio_embedding}')
    #     return audio_embedding

    # def encode_audio(self, audio: Union[torch.Tensor, np.ndarray]) -> torch.Tensor:
    # # 필요하다면 numpy 배열을 torch 텐서로 변환
    ...
    # def encode_audio(self, audio: Union[torch.Tensor, np.ndarray]) -> torch.Tensor:
    # # audio가 NumPy 배열이거나 리스트일 경우, PyTorch 텐서로 변환
    #     if not isinstance(audio, torch.Tensor):
    #         audio = torch.tensor(audio, dtype=torch.float32)
        
    #     # 변환된 텐서를 적절한 디바이스로 이동
    #     audio = audio.to(self.device)
        
    #     # 오디오 인코딩 수행
    #     audio_embedding = self.audio(audio)
    #     return audio_embedding
    
    def encode_audio(self, audio: List[np.ndarray]) -> torch.Tensor:
    # audio 매개변수가 리스트 형태의 오디오 데이터를 받음
    # 각 오디오 샘플을 PyTorch 텐서로 변환하고, 해당 디바이스로 이동한 뒤, 오디오 인코딩을 수행
    
        encoded_audios = []
        
        for audio_sample in audio:
            # 리스트의 각 오디오 샘플을 PyTorch 텐서로 변환
            if isinstance(audio_sample, np.ndarray):
                audio_tensor = torch.from_numpy(audio_sample).float()  # 데이터 타입을 float으로 지정
            elif isinstance(audio_sample, torch.Tensor):
                audio_tensor = audio_sample.float()  # 이미 텐서인 경우 데이터 타입만 조정
            else:
            ...
            # 오디오 인코딩 수행
            audio_embedding = self.audio(audio_tensor.unsqueeze(0))  # 배치 차원 추가
            encoded_audios.append(audio_embedding)
        
        # 인코딩된 오디오 특징들을 하나의 텐서로 결합
        encoded_audios_tensor = torch.cat(encoded_audios, dim=0)
        
        return encoded_audios_tensor
